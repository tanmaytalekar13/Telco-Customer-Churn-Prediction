...
Losing a customer is always more costly than retaining one — so being able to predict *who might leave*
before they actually do is incredibly valuable.

This project builds a churn prediction system using three gradient boosting models: **XGBoost**, **CatBoost**, and **LightGBM**.
Each model was trained and tuned on real telecom customer data, and this app lets you interact with them directly.

## How to use this app

1. Pick a model from the **sidebar** — XGBoost, CatBoost, or LightGBM
2. Hit **`Performance on Test Dataset`** to see how the model performs (ROC AUC + curve)
3. Hit **`Prediction on Random Instance`** to test on a random customer from the test set
4. Or fill in customer details manually in the sidebar and click **`Predict`** for a live prediction
5. All results appear in the **[Prediction Result](#prediction-result)** section below
...
def make_prediction(X_test):
    # Convert to numpy to avoid feature name mismatches across model versions
    X_array = X_test.values if hasattr(X_test, 'values') else X_test
    if hasattr(clf, 'predict_proba'):
        # XGBoost and CatBoost
        test_pred = clf.predict_proba(X_array)[:, 1]
    else:
        # LightGBM Booster loaded via lgbm.Booster has no predict_proba
        test_pred = clf.predict(X_array)
    return test_pred
...
user_df = user_df[X.columns]  # align column order to training set
